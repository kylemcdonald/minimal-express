...
function draw(){
	...
	var cy = +(mx - (width / 2)) / bigSide;
	var cx = -(my - (height / 2)) / bigSide;

	// var orientation = gyro.getOrientation();
	// document.getElementById('x').innerHTML = orientation.x.toFixed(3);
	// document.getElementById('y').innerHTML = orientation.y.toFixed(3);
	// document.getElementById('z').innerHTML = orientation.z.toFixed(3);
	cy = smoothed.x/8;
	cx = (smoothed.y - smoothed.z)/16;

	var maxDistance = 1;
	var dist = Math.sqrt(cx * cx + cy * cy);
	// if(dist > maxDistance || mx == 0 || my == 0) {
	// 	cx = 0, cy = 0;
	// }

	var range = 0.5;
	cy = constrain(cy, -range, +range);
	cx = constrain(cx, 0, +range);
